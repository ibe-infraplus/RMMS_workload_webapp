...
1. รันเซิร์ฟเวอร์ด้วยคำสั่ง: `python -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload`
2. เปิดเบราว์เซอร์ไปที่: **`http://localhost:8000/docs`**
...
หากต้องการทดสอบผ่าน Command Line สามารถยิงคำสั่งเหล่านี้ได้เลย (ตัวอย่างพอร์ต 8000):

### 📌 2.1 ดึงรายชื่อแขวงทั้งหมด
เส้นนี้ใช้สำหรับทำ Dropdown เลือกแขวงในหน้า Frontend
```bash
curl -X 'GET' 'http://localhost:8000/api/districts' -H 'accept: application/json'
```
**ตัวอย่าง Response:**
```json
{
...
```bash
curl -X 'GET' 'http://localhost:8000/api/districts/100' -H 'accept: application/json'
```
...
```bash
curl -X 'GET' 'http://localhost:8000/api/config' -H 'accept: application/json'
```
...
```bash
curl -X 'POST' \
  'http://localhost:8000/api/calculate/100' \
...
*   **วิธีแก้:** ตรวจสอบว่าพอร์ตที่รัน `uvicorn` ตรงกับใน URL หรือไม่ (บางเครื่องอาจเด้งไปพอร์ต 8001 ถ้า 8000 ไม่ว่าง) และให้เช็คว่ามี `/api/` นำหน้าเสมอ
